...
import { CreateCC2, GetCCByUser, GetCCById, EditCC, DeleteCC } from "../../services/credit-card.service";
...
const AdminCreditCards = () => {
    ...
    useEffect(() => {
        ...
        GetUserById(payload.id).then((json) => {
            ...
            setCC({
                ...
                CardHolder: `${json.response.nombre.split(" ")[0]} ${json.response.apellido.split(" ")[0]}`
            });
    ...
    const CreditCardCard = ({ cc }) => {
        return (
            <Card sx={{ m: 2, p: 1, width: 300 }}>
                ...
                <ListItemButton
                    sx={{ display: "flex", justifyContent: "center", gap: 1 }}
                    onClick={() => { handleConfirmOpen(cc.id) }}
                >
    ...
    return (
        <Box sx={{ mt: 12, display: "flex", flexDirection: "column", alignItems: "center" }}>
            ...
            />
            <Box>
                <Button variant="contained" color="success" onClick={handleCreateOpen}>Agregar Nueva Tarjeta</Button>
            </Box>
            <Box sx={{ display: "flex", flexWrap: "wrap", justifyContent: "center", mt: 3 }}>
                {
                    ccList.length > 0 ? (ccList.map(cc => <CreditCardCard cc={cc} />)) : <Typography variant="h6" component="h2">
